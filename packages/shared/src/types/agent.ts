...
export const BUILT_IN_AGENTS: BuiltInAgentMeta[] = [
  ...BUILT_IN_AGENT_MANIFESTS.map((agent) => ({
    id: agent.id,
    name: agent.name,
  ...
  })),
  {
    id: "justice",
    name: "Justice",
    description: "Tarot Game realism judge. Owns adjudication and dice checks before narration.",
  ...
  },
  {
    id: "emperor",
    name: "Emperor",
    description: "Tarot Game scenario composer. Owns turn beats and game engine directives.",
    author: DEFAULT_AGENT_AUTHOR,
    phase: "pre_generation",
    enabledByDefault: false,
    category: "writer",
  ...
  },
  {
    id: "chariot",
    name: "Chariot",
    description: "Tarot Game HUD widget updater. Reads the finished turn and updates active widgets.",
    author: DEFAULT_AGENT_AUTHOR,
    phase: "post_processing",
    enabledByDefault: false,
    category: "tracker",
...
export const DEFAULT_AGENT_TOOLS: Record<string, string[]> = {
  ...Object.fromEntries(BUILT_IN_AGENT_MANIFESTS.map((agent) => [agent.id, [...(agent.defaultTools ?? [])]])),
  justice: [],
  emperor: [],
  hermit: [],
  chariot: [],
