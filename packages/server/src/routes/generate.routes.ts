      ...
      while (true) {
        ...
        if (chatMode === "game") {
          ...
          tarotAgentContextPackets = {
            justiceAdjudication: {
              ...
              tone: gmCtx.tone,
              map: compactGameMapForTarot(gmCtx.map),
              npcs: compactGameNpcsForTarot(gmCtx.npcs),
              partyNames: gmCtx.partyNames,
              presentCharacterNames: gmCtx.presentCharacterNames,
              playerInventory: normalizeGameInventoryForTarot(chatMeta.gameInventory),
              playerNotes: gmCtx.playerNotes,
              gameTime: gmCtx.gameTime,
              weatherContext: gmCtx.weatherContext,
            },
            emperorComposition: {
              gameActiveState: gmCtx.gameActiveState,
              storyArc: gmCtx.storyArc,
              plotTwists: gmCtx.plotTwists,
              campaignPlan: gmCtx.campaignPlan,
              map: compactGameMapForTarot(gmCtx.map),
              npcs: compactGameNpcsForTarot(gmCtx.npcs),
              partyNames: gmCtx.partyNames,
              presentCharacterNames: gmCtx.presentCharacterNames,
              referenceCharacterIndex: gmCtx.referenceCharacterIndex,
              inventory: normalizeGameInventoryForTarot(chatMeta.gameInventory),
              playerNotes: gmCtx.playerNotes,
        ...
        if (shouldRunDirectorSecretPlot || shouldRunPreGen || shouldRunKR || shouldRunRouter) {
          ...
          if (plotResult?.success && plotResult.data && typeof plotResult.data === "object") {
            ...
            const agentConfigId = directorSecretPlotAgent?.id ?? plotResult.agentId;
