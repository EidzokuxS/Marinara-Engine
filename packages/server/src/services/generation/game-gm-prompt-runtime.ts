...
import { buildGmSystemPrompt, buildTowerNarrativeSystemPrompt, type GmPromptContext } from "../game/gm-prompts.js";
...
function buildCharacterReferenceIndexEntry(characterData: Record<string, any>): { name: string; entry: string } | null {
  const { name } = buildLibraryCardParts(characterData);
  const source =
  ...
  return name ? { name, entry: `${name}: ${shortDescription}` } : null;
}

function buildCharacterReferencePromptCard(characterData: Record<string, any>): { name: string; card: string } | null {
  const { name, parts } = buildLibraryCardParts(characterData);
  if (!name) return null;
  const scenario = cardPromptText(characterData.scenario);
  const creatorNotes = cardPromptText(characterData.creator_notes);
  const systemPrompt = cardPromptText(characterData.system_prompt);
  const postHistoryInstructions = cardPromptText(characterData.post_history_instructions);
  const exampleDialogue = cardPromptText(characterData.mes_example);
  if (scenario) parts.push(`Scenario: ${scenario}`);
  if (creatorNotes) parts.push(`Creator Notes: ${creatorNotes}`);
  ...
  if (exampleDialogue) parts.push(`Example Dialogue: ${exampleDialogue}`);
...
function parsePresentCharacterNames(value: unknown): string[] {
  ...
  return parsed
    ...
    .filter(
      (name, index, names) =>
        name.length > 0 && names.findIndex((other) => normalizeTextForMatch(other) === normalizeTextForMatch(name)) === index,
    );
}
...
export async function injectGameGmPromptRuntime(args: {
  ...
  try {
    if (latestSnapshot) {
      if (latestSnapshot.weather)
        weatherContext = `Current weather: ${latestSnapshot.weather}${latestSnapshot.temperature ? `, ${latestSnapshot.temperature}` : ""}`;
      if (latestSnapshot.time || latestSnapshot.date) gameTime = [latestSnapshot.date, latestSnapshot.time].filter(Boolean).join(", ");
    }
  } catch {
    /* ignore */
  }
  ...
  const presentCharacterNames = parsePresentCharacterNames(latestSnapshot?.presentCharacters);
  const presentReferenceNames = new Set(presentCharacterNames.map((name) => normalizeTextForMatch(name)));
  const referenceCharacterIds = Array.from(
    new Set(
      [
        ...(Array.isArray(args.chatMetadata.gameReferenceCharacterIds)
          ? (args.chatMetadata.gameReferenceCharacterIds as unknown[])
          : []),
        ...(Array.isArray(setupConfig?.referenceCharacterIds)
          ? ((setupConfig.referenceCharacterIds as unknown[]) ?? [])
          : []),
  ...
  for (const referenceCharacterId of referenceCharacterIds) {
    try {
      const referenceCharacter = await args.chars.getById(referenceCharacterId);
      if (!referenceCharacter) continue;
      const referenceData = parseMaybeJson(referenceCharacter.data) as Record<string, any> | null;
      if (!referenceData || typeof referenceData !== "object") continue;
      const indexEntry = buildCharacterReferenceIndexEntry(referenceData);
      if (!indexEntry) continue;
      referenceCharacterIndex.push(indexEntry.entry);
      if (!presentReferenceNames.has(normalizeTextForMatch(indexEntry.name))) continue;
      const promptCard = buildCharacterReferencePromptCard(referenceData);
      if (promptCard) referenceCharacterCards.push(promptCard);
    } catch {
  ...
  const gmCtx: GmPromptContext = {
    ...
    gameSpecialInstructions:
      typeof args.chatMetadata.gameSpecialInstructions === "string"
        ...
        : null,
    chariotHandlesWidgets: args.towerNarrativeOnly,
    justiceHandlesAdjudication: args.towerNarrativeOnly,
  };

  const builtGmPrompt = args.towerNarrativeOnly ? buildTowerNarrativeSystemPrompt(gmCtx) : buildGmSystemPrompt(gmCtx);
