...
function buildAgentMessages(
  ...
  if (requiresTerminalUserInstruction) {
    const instruction =
      finalParts.length > 0 && agentResponseIsJson({ type: agentType, settings: {} })
...
export function buildAgentExtras(context: AgentContext, agentTypes: string[] = []): string {
  ...
  if (agentTypes.includes("emperor") && typeof context.memory._emperorRepairInstruction === "string") {
...
function isTextOutputAgentType(agentType: string): boolean {
  return !JSON_AGENTS.has(agentType) && TEXT_RESULT_TYPES.has(AGENT_RESULT_TYPE_MAP[agentType] ?? "context_injection");
}
